...
void UDT_Sender::send_file(QString file)
{
	...
	if (0 != getaddrinfo(QString("%1").arg(this->ip_server).toStdString().c_str(), QString("%1").arg(this->port_server).toStdString().c_str(), &hints, &peer))
	{
		...
		freeaddrinfo(peer); // 释放分配的内存
		return;
	}
	// 显式绑定并将套接字与相应地址绑定
	if (UDT::ERROR == UDT::bind(client, peer->ai_addr, peer->ai_addrlen))
	{
		emit this->sig_log("绑定client失败\n");
	...
	while (run_flag)
	{
		...
		if (this->sended_bytes >= this->current_total_bytes || byte.isEmpty())
		{
			break;
		}
		QThread::msleep(1);
